...
module Gio
  ...
  class ConverterInputStream < FilterInputStream
    ...
    GICrystal.declare_new_method(ConverterInputStream, g_object_get_qdata)

    # Initialize a new `ConverterInputStream`.
    def initialize
      super
    end
    ...
    def self.g_type : UInt64
      @@g_type ||= LibGio.g_converter_input_stream_get_type.tap do |g_type|
        # Set the Crystal constructor on type qdata
        ctor = ->Gio::ConverterInputStream.new(Void*, GICrystal::Transfer)
    ...
    def converter : Gio::Converter
      ...
      # C call
      _retval = LibGio.g_converter_input_stream_get_converter(to_unsafe)
