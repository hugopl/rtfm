...
module Soup
  ...
  class AuthNegotiate < Auth
    ...
    GICrystal.declare_new_method(AuthNegotiate, g_object_get_qdata)

    # Initialize a new `AuthNegotiate`.
    def initialize
      super
    end
    ...
    def self.g_type : UInt64
      @@g_type ||= LibSoup.soup_auth_negotiate_get_type.tap do |g_type|
        # Set the Crystal constructor on type qdata
        ctor = ->Soup::AuthNegotiate.new(Void*, GICrystal::Transfer)
        LibGObject.g_type_set_qdata(g_type, GICrystal::INSTANCE_FACTORY, ctor.pointer)
      end
    end

    def self.supported : Bool
      # soup_auth_negotiate_supported: (None)
      # Returns: (transfer none)

      # C call
      _retval = LibSoup.soup_auth_negotiate_supported
