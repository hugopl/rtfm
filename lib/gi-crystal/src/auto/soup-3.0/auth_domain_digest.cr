...
module Soup
  ...
  class AuthDomainDigest < AuthDomain
    ...
    GICrystal.declare_new_method(AuthDomainDigest, g_object_get_qdata)

    # Initialize a new `AuthDomainDigest`.
    def initialize
      super
    end
    ...
    def initialize(*, auth_callback : Soup::AuthDomainDigestAuthCallback? = nil, auth_data : Pointer(Void)? = nil, filter : Soup::AuthDomainFilter? = nil, filter_data : Pointer(Void)? = nil, generic_auth_callback : Soup::AuthDomainGenericAuthCallback? = nil, generic_auth_data : Pointer(Void)? = nil, proxy : Bool? = nil, realm : ::String? = nil)
    ...
    def self.g_type : UInt64
      @@g_type ||= LibSoup.soup_auth_domain_digest_get_type.tap do |g_type|
        # Set the Crystal constructor on type qdata
        ctor = ->Soup::AuthDomainDigest.new(Void*, GICrystal::Transfer)
        LibGObject.g_type_set_qdata(g_type, GICrystal::INSTANCE_FACTORY, ctor.pointer)
      end
    end

    def auth_callback=(value : Soup::AuthDomainDigestAuthCallback?) : Soup::AuthDomainDigestAuthCallback?
    ...
    def auth_callback : Soup::AuthDomainDigestAuthCallback?
      # Returns: None

      value = uninitialized Pointer(Void)
      LibGObject.g_object_get(self, "auth-callback", pointerof(value), Pointer(Void).null)
      Soup::AuthDomainDigestAuthCallback.new(value, GICrystal::Transfer::None) unless value.null?
    end
    ...
    def self.encode_password(username : ::String, realm : ::String, password : ::String) : ::String
      # soup_auth_domain_digest_encode_password: (None)
      # @username:
      # @realm:
      # @password:
      ...
      # C call
      _retval = LibSoup.soup_auth_domain_digest_encode_password(username, realm, password)

      # Return value handling
      GICrystal.transfer_full(_retval)
    end

    def auth_callback=(callback : Soup::AuthDomainDigestAuthCallback) : Nil
      # soup_auth_domain_digest_set_auth_callback: (Method | Setter)
      # @callback:
      # @user_data: (nullable)
      # @dnotify:
      ...
      if callback
        ...
        callback = ->(lib_domain : Pointer(Void), lib_msg : Pointer(Void), lib_username : Pointer(LibC::Char), lib_user_data : Pointer(Void)) {
          # Generator::BuiltInTypeArgPlan
          domain = Soup::AuthDomainDigest.new(lib_domain, GICrystal::Transfer::None)
          # Generator::BuiltInTypeArgPlan
      ...
      # C call
      LibSoup.soup_auth_domain_digest_set_auth_callback(to_unsafe, callback, user_data, dnotify)
