...
module Soup
  ...
  class CookieJarText < CookieJar
    ...
    GICrystal.declare_new_method(CookieJarText, g_object_get_qdata)

    # Initialize a new `CookieJarText`.
    def initialize
      super
    end
    ...
    def self.g_type : UInt64
      @@g_type ||= LibSoup.soup_cookie_jar_text_get_type.tap do |g_type|
        # Set the Crystal constructor on type qdata
        ctor = ->Soup::CookieJarText.new(Void*, GICrystal::Transfer)
    ...
    def self.new(filename : ::String, read_only : Bool) : self
      # soup_cookie_jar_text_new: (Constructor)
      # @filename:
      # @read_only:
      # Returns: (transfer full)

      # C call
      _retval = LibSoup.soup_cookie_jar_text_new(filename, read_only)

      # Return value handling
      Soup::CookieJarText.new(_retval, GICrystal::Transfer::Full)
    end
