...
module Adw
  ...
  class Spinner < Gtk::Widget
    ...
    def self.g_type : UInt64
      @@g_type ||= LibAdw.adw_spinner_get_type.tap do |g_type|
        # Set the Crystal constructor on type qdata
        ctor = ->Adw::Spinner.new(Void*, GICrystal::Transfer)
        LibGObject.g_type_set_qdata(g_type, GICrystal::INSTANCE_FACTORY, ctor.pointer)
      end
    end

    def self.new : self
      # adw_spinner_new: (Constructor)
      # Returns: (transfer none)

      # C call
      _retval = LibAdw.adw_spinner_new

      # Return value handling
      Adw::Spinner.new(_retval, GICrystal::Transfer::Full)
    end
