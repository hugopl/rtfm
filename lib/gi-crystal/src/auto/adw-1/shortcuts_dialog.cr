...
module Adw
  ...
  class ShortcutsDialog < Dialog
    ...
    GICrystal.declare_new_method(ShortcutsDialog, g_object_get_qdata)

    # Initialize a new `ShortcutsDialog`.
    def initialize
      super
    end
    ...
    def self.g_type : UInt64
      @@g_type ||= LibAdw.adw_shortcuts_dialog_get_type.tap do |g_type|
        # Set the Crystal constructor on type qdata
        ctor = ->Adw::ShortcutsDialog.new(Void*, GICrystal::Transfer)
        LibGObject.g_type_set_qdata(g_type, GICrystal::INSTANCE_FACTORY, ctor.pointer)
      end
    end

    def self.new : self
      # adw_shortcuts_dialog_new: (Constructor)
      # Returns: (transfer none)

      # C call
      _retval = LibAdw.adw_shortcuts_dialog_new

      # Return value handling
      Adw::ShortcutsDialog.new(_retval, GICrystal::Transfer::Full)
    end

    def add(section : Adw::ShortcutsSection) : Nil
      # adw_shortcuts_dialog_add: (Method)
      ...
      # C call
      LibAdw.adw_shortcuts_dialog_add(to_unsafe, section)
