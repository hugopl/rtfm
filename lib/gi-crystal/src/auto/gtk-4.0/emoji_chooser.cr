  ...
  class EmojiChooser < Popover
    ...
    def self._register_derived_type(class_name : String, class_init, instance_init)
      LibGObject.g_type_register_static_simple(g_type, class_name,
        sizeof(LibGtk::EmojiChooserClass), class_init,
        sizeof(LibGtk::EmojiChooser), instance_init, 0)
    end

    GICrystal.declare_new_method(EmojiChooser, g_object_get_qdata)

    # Initialize a new `EmojiChooser`.
    def initialize
      super
    end
    ...
    def self.g_type : UInt64
      @@g_type ||= LibGtk.gtk_emoji_chooser_get_type.tap do |g_type|
        # Set the Crystal constructor on type qdata
        ctor = ->Gtk::EmojiChooser.new(Void*, GICrystal::Transfer)
    ...
    def self.new : self
      ...
      Gtk::EmojiChooser.new(_retval, GICrystal::Transfer::Full)
    end

    struct EmojiPickedSignal < GObject::Signal
      def name : String
      ...
      def connect(handler : Proc(Gtk::EmojiChooser, ::String, Nil), *, after : Bool = false) : GObject::SignalConnection
        _box = ::Box.box(handler)
        handler = ->(_lib_sender : Pointer(Void), lib_text : Pointer(LibC::Char), _lib_box : Pointer(Void)) {
          _sender = Gtk::EmojiChooser.new(_lib_sender, GICrystal::Transfer::None)
          # Generator::BuiltInTypeArgPlan
          text = ::String.new(lib_text)
          ::Box(Proc(Gtk::EmojiChooser, ::String, Nil)).unbox(_lib_box).call(_sender, text)
        }.pointer
