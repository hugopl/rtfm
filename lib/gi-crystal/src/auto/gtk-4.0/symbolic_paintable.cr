module Gtk
  module SymbolicPaintable
    ...
    def snapshot_with_weight(snapshot : Gdk::Snapshot, width : Float64, height : Float64, colors : Enumerable(Gdk::RGBA), weight : Float64) : Nil
      # gtk_symbolic_paintable_snapshot_with_weight: (Method)
      # @snapshot:
      # @width:
      # @height:
      # @colors: (array length=n_colors element-type Interface)
      ...
      # C call
      LibGtk.gtk_symbolic_paintable_snapshot_with_weight(to_unsafe, snapshot, width, height, colors, n_colors, weight)
...
# @n_colors: 
    ...
    # @weight:
    # Returns: (transfer none)
    private macro _register_snapshot_with_weight_vfunc(impl_method_name)
      private def self._vfunc_snapshot_with_weight(%this : Pointer(Void), lib_snapshot :  Pointer(Void), lib_width :  Float64, lib_height :  Float64, lib_colors :  Pointer(LibGdk::RGBA), lib_n_colors :  UInt64, lib_weight :  Float64, ) : Void
        # @snapshot: 
# @width: 
# @height: 
# @colors: (array length=n_colors element-type Interface)
...
weight=lib_weight
        ...
        %retval = %instance.as(self).{{ impl_method_name.id }}(snapshot, width, height, colors, n_colors, weight)
      ...
      def self._install_iface_Gtk__SymbolicPaintable(type_struct : Pointer(LibGObject::TypeInterface)) : Nil
        ...
        vfunc_ptr.value = (->_vfunc_snapshot_with_weight(Pointer(Void), Pointer(Void), Float64, Float64, Pointer(LibGdk::RGBA), UInt64, Float64)).pointer
        previous_def
      end
    end
    ...
    # @weight:
    # Returns: (transfer none)
    private macro _register_unsafe_snapshot_with_weight_vfunc(impl_method_name)
      private def self._vfunc_unsafe_snapshot_with_weight(%this : Pointer(Void), lib_snapshot :  Pointer(Void), lib_width :  Float64, lib_height :  Float64, lib_colors :  Pointer(LibGdk::RGBA), lib_n_colors :  UInt64, lib_weight :  Float64, ) : Void
# @snapshot: 
# @width: 
# @height: 
# @colors: (array length=n_colors element-type Interface)
...
# @weight: 
        ...
        %instance.as(self).{{ impl_method_name.id }}(lib_snapshot, lib_width, lib_height, lib_colors, lib_n_colors, lib_weight)
      end

      def self._install_iface_Gtk__SymbolicPaintable(type_struct : Pointer(LibGObject::TypeInterface)) : Nil
        vfunc_ptr = (type_struct.as(Pointer(Void)) + 24).as(Pointer(Pointer(Void)))
        @@_gi_parent_vfunc_snapshot_with_weight = Proc(Pointer(Void), Pointer(Void), Float64, Float64, Pointer(LibGdk::RGBA), UInt64, Float64, Void).new(vfunc_ptr.value, Pointer(Void).null) unless vfunc_ptr.value.null?
        vfunc_ptr.value = (->_vfunc_unsafe_snapshot_with_weight(Pointer(Void), Pointer(Void), Float64, Float64, Pointer(LibGdk::RGBA), UInt64, Float64)).pointer
        previous_def
      end

      @@_gi_parent_vfunc_snapshot_with_weight : Proc(Pointer(Void), Pointer(Void), Float64, Float64, Pointer(LibGdk::RGBA), UInt64, Float64, Void)? = nil
    end
