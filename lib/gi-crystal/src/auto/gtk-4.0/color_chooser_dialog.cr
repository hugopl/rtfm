...
module Gtk
  ...
  class ColorChooserDialog < Dialog
    ...
    GICrystal.declare_new_method(ColorChooserDialog, g_object_get_qdata)

    # Initialize a new `ColorChooserDialog`.
    def initialize
      super
    end
    ...
    def self.g_type : UInt64
      @@g_type ||= LibGtk.gtk_color_chooser_dialog_get_type.tap do |g_type|
        # Set the Crystal constructor on type qdata
        ctor = ->Gtk::ColorChooserDialog.new(Void*, GICrystal::Transfer)
    ...
    def self.new(title : ::String?, parent : Gtk::Window?) : self
      # gtk_color_chooser_dialog_new: (Constructor)
      # @title: (nullable)
      # @parent: (nullable)
      ...
      # C call
      _retval = LibGtk.gtk_color_chooser_dialog_new(title, parent)

      # Return value handling
      Gtk::ColorChooserDialog.new(_retval, GICrystal::Transfer::Full)
    end
