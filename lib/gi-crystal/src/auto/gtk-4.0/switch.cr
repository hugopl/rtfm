...
module Gtk
  ...
  class Switch < Widget
    ...
    GICrystal.declare_new_method(Switch, g_object_get_qdata)
    ...
    def self.g_type : UInt64
      @@g_type ||= LibGtk.gtk_switch_get_type.tap do |g_type|
        # Set the Crystal constructor on type qdata
        ctor = ->Gtk::Switch.new(Void*, GICrystal::Transfer)
    ...
    def self.new : self
      ...
      Gtk::Switch.new(_retval, GICrystal::Transfer::Full)
    end
    ...
    def active : Bool
      ...
      # C call
      _retval = LibGtk.gtk_switch_get_active(to_unsafe)
    ...
    def state : Bool
      ...
      # C call
      _retval = LibGtk.gtk_switch_get_state(to_unsafe)
    ...
    def active=(is_active : Bool) : Nil
      ...
      # C call
      LibGtk.gtk_switch_set_active(to_unsafe, is_active)
    ...
    def state=(state : Bool) : Nil
      ...
      # C call
      LibGtk.gtk_switch_set_state(to_unsafe, state)
    ...
    struct ActivateSignal < GObject::Signal
      ...
      def connect(handler : Proc(Gtk::Switch, Nil), *, after : Bool = false) : GObject::SignalConnection
        _box = ::Box.box(handler)
        handler = ->(_lib_sender : Pointer(Void), _lib_box : Pointer(Void)) {
          _sender = Gtk::Switch.new(_lib_sender, GICrystal::Transfer::None)
          ::Box(Proc(Gtk::Switch, Nil)).unbox(_lib_box).call(_sender)
        }.pointer
    ...
    struct StateSetSignal < GObject::Signal
      def name : String
